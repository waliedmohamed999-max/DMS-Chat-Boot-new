...
import { takeOverTenantChatSession, sendTenantStaffReply } from "./actions";

type SenderType = "VISITOR" | "AI" | "STAFF";
type SessionStatus = "OPEN" | "HANDED_OFF";

export type TenantChatSessionSummary = {
  id: string;
  tenantId: string;
...
const STATUS_BADGE: Record<SessionStatus, { label: string; className: string }> = {
  ...
  HANDED_OFF: { label: "مُحوَّلة لفريقك", className: "bg-warning-500/10 text-warning-500" },
};

const SENDER_LABEL: Record<SenderType, string> = { VISITOR: "الزائر", AI: "الموظف الذكي", STAFF: "أنت" };

/** نظير ChatSessionCard في admin/chats بالحرف، لكن يستطلع /api/tenant-chat/{tenantId}/{sessionId}/messages
 * (بيانات تاجر خاضعة لـRLS) بدل /api/platform-chat/{sessionId}/messages، وبلا شارة "عرض تجريبي". */
export function TenantChatSessionCard({ session }: { session: TenantChatSessionSummary }) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const url = `/api/tenant-chat/${session.tenantId}/${session.id}/messages${lastMessageIdRef.current ? `?after=${lastMessageIdRef.current}` : ""}`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok) return;
        const data = await res.json();
        if (data.status) setStatus(data.status);
        const newMessages: TenantChatSessionSummary["messages"] = data.messages ?? [];
  ...
  function handleTakeOver() {
    startTransition(() => takeOverTenantChatSession(session.id));
  ...
  function handleReply() {
    ...
    startTransition(() => sendTenantStaffReply(session.id, text));
  }
